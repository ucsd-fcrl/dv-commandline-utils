...
public:
  ...
  void Increment()
  {
    ...
    for (unsigned int i = 0; i < this->m_MeshDirectories.size(); ++i) {
      this->m_MeshViews.at(i).Update(
        this->GetCurrentMeshFileName(this->m_MeshDirectories.at(i)));
    }
  ...
  {
    return this->m_SegmentationDirectory +
           std::to_string(this->index.GetCurrent()) + ".nii.gz";
  }
  ...
  void SetupSegmentations(std::vector<unsigned int> Labels,
                          std::vector<std::array<double, 3>> Colors,
                          vtkRenderer* Renderer)
  {

    auto progress =
      dv::Progress(this->index.GetRange() - this->index.GetStart());
...
main(int argc, char** argv)
{
  ...
  description.add_options()("help", "Print usage information.")(
    ...
    "Downsampling factor.")("window-size",
  ...
  const std::string sd = dv::AppendCharacterIfAbsent(
    vm["segmentation-directory"].as<std::string>(), '/');
  const std::vector<unsigned int> labels =
    vm["labels"].as<std::vector<unsigned int>>();
  const double SampleRate = vm["downsampling-factor"].as<double>();
  const unsigned int WindowSize = vm["window-size"].as<unsigned int>();
  const unsigned int NumberOfFiles = dv::NumberOfSequentialFiles(
    [sd](size_t n) { return sd + std::to_string(n) + ".nii.gz"; });
  ...
  style->SetupSegmentations(
    labels, dv::GetListOfColors(), style->GetCurrentRenderer());
