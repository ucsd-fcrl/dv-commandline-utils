...
- Partitioning such that all elements < pivot come before and all elements >
pivot come after.
- Recursively apply the above steps to the sub-array of elements with smaller
values and separately to the sub-array of elements with greater values (neither
subarray includes pivot).


Time Efficiency:
- Worst Case: O(n^2) -- Occurs if the worst pivot possible is always chosen.
- Best Case: O(n log(n)) -- Occurs if the pivot is always chosen to be the
median.
- Average Case: O(n log(n)) -- Occurs if the sequence is sufficiently random.

Notes:
- The worst case really matters in this case, as semi-sorted sequences are very
common in practice.
*/
...
void
QuickSort(const It b, const It e)
{
